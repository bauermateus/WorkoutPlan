...
import com.mbs.workoutplan.databinding.FragmentEditProfileBinding
...
class EditProfileFragment : Fragment() {
    ...
    private fun setup() {
        args.userData.image?.let { binding.img.load(it) }
        args.userData.name?.let { binding.name.setText(it) }
    }

    private fun onClick() {
        binding.confirmButton.setOnClickListener {
            if (binding.name.text.isNullOrBlank()) {
                toast("Nome precisa ser preenchido.")
                return@setOnClickListener
            }
            if (binding.name.text.toString()
                    .trim() == args.userData.name && selectedImgUri == null
            ) {
                findNavController().popBackStack()
            }
            viewModel.updateData(binding.name.text.toString(), selectedImgUri)
        }
    ...
    private fun processEvents(event: EditProfileEvent) {
        when (event) {
